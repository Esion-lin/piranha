...
__global__ void ringMultiplication(T *a, T *b, T *c,
    ...
    if (ROW < data_len && COL < degree) {
        ...
        for(int k = COL+1; k < degree; k++){
            int a_idx = ROW * degree + degree+COL - k;
            int b_idx = ROW * degree + k;
            c[ROW * degree + COL] -= a[a_idx] * b[b_idx];
        }
    }
...
template<typename T>
__global__ void mssOffline(T *r_x, T *r_y, T *r_a, T*gamma, T*ss1, T*ss2,T*ss3,
        int data_len, int degree) {
            //mode X^degree + 1
    int ROW = blockIdx.y*blockDim.y+threadIdx.y;
    int COL = blockIdx.x*blockDim.x+threadIdx.x;
    ...
    if (ROW < data_len && COL < degree) {
        for(int k = 0; k <= COL; k++){
            ...
            int a_idx = ROW * degree + COL - k;
            int b_idx = ROW * degree + k;
            ss1[ROW * degree + COL] += r_x[a_idx] * r_y[b_idx];
            ss2[ROW * degree + COL] += r_x[a_idx] * r_a[b_idx];
            ss3[ROW * degree + COL] += r_a[a_idx] * r_y[b_idx];
        ...
        for(int k = COL+1; k < degree; k++){
            for(int w = 0; w <= k; w++){
                int a_idx = ROW * degree + k - w;
                int b_idx = ROW * degree + w;
                int c_idx = ROW * degree + degree+COL - k;
                gamma[COL] -= r_x[a_idx] * r_y[b_idx] * r_a[c_idx];
...
template<typename T>
__global__ void mssOnline(const T *ss1, const T *ss2, const T *ss3, const T *m_x, const T *m_y, T *m_a, T*gamma, T*output,
        int data_len, int degree) {
    //ss : joint item
    int ROW = blockIdx.y*blockDim.y+threadIdx.y;
    int COL = blockIdx.x*blockDim.x+threadIdx.x;
    ...
    if (ROW < data_len && COL < degree) {
        for(int k = 0; k <= COL; k++){
            ...
            int a_idx = ROW * degree + COL - k;
            int b_idx = ROW * degree + k;
            output[COL] += m_x[a_idx] * m_y[b_idx] + ss1[a_idx] * m_a[b_idx] + ss2[a_idx] * m_y[b_idx] + ss3[a_idx] * m_x[b_idx];
...
__global__ void ringInterpolation_2d(T *delta, T *h1, T *h2, T *h3, T *h, 
    ...
    if (ROW < data_len && COL < degree) {
        ...
        for(int k = COL+1; k < degree; k++){
            for(int w = 0; w < k; w++){
                int a_idx = ROW * degree + degree + k - w;
                int b_idx = ROW * degree + w;
                int c_idx = ROW * degree + COL - k;
                if(COL - k == 0){
                    delt1 = delta[ COL - k] - 1;
...
void ringDot(
    ...
    kernel::ringDotProduct<<<blocksPerGrid,threadsPerBlock>>>(
        thrust::raw_pointer_cast(&a->begin()[0]),
        thrust::raw_pointer_cast(&b->begin()[0]),
        thrust::raw_pointer_cast(&c->begin()[0]),
        data_len, degree
...
template<typename T, typename I>
void mssOffline(
        const DeviceData<T, I> *r_x, const DeviceData<T, I> *r_y, DeviceData<T, I> *r_a, DeviceData<T, I> *gamma, DeviceData<T, I> *ss1, DeviceData<T, I> *ss2, DeviceData<T, I> *ss3,
        size_t data_len, size_t degree) {
    ...
    kernel::mssOffline<<<blocksPerGrid,threadsPerBlock>>>(
        ...
        thrust::raw_pointer_cast(&ss1->begin()[0]),
        thrust::raw_pointer_cast(&ss2->begin()[0]),
        thrust::raw_pointer_cast(&ss3->begin()[0]),
        data_len, degree
    );
...
template<typename T, typename I>
void mssOnline(
        const DeviceData<T, I> *ss1,  const DeviceData<T, I> *ss2,  const DeviceData<T, I> *ss3, const DeviceData<T, I> *m_x, const DeviceData<T, I> *m_y,  DeviceData<T, I> *m_a, DeviceData<T, I> *gamma, DeviceData<T, I> *output,
        size_t data_len, size_t degree) {
